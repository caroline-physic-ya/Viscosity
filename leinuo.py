...
def calculate_two():
    ...
    var_vis_final = tk.StringVar()
    vis = tk.Label(window_new2, textvariable=var_vis_final, bg='yellow', fg='green', font=('Arial', 14), width=35,
                   height=2).place(x=10, y=350)
    ...
    def calculate_vis(t, L, rho, rhoye, d, D, H):
        ...
        if ((vis_key) >= 0):
            ...
            if ((re_key < 0.1) & (re_key >=0)):
                ...
                var_re.set('雷诺系数' + re)
                var_vis_final.set('粘滞系数无需修正')
            elif ((re_key >0.1) & (re_key <= 1)):
                re = d * (L * 0.01 / t) * rhoye / (vis_key)
                vis_key=round((vis_key),5)

                vis_xiuzheng = (vis_key) * (1 + (0.1875 * re))
                vis_xiuzheng = str(round(vis_xiuzheng),5)
                re = str(re)
                vis_key=str(vis_key)
                var_vis.set('粘滞系数为：' + vis_key)
                var_re.set('雷诺系数' + re)
                var_vis_final.set('修正后的粘滞系数：'+vis_xiuzheng)
            elif ((re_key >= 1) & (re_key <= 2)):
                re = d * (L * 0.01 / t) * rhoye / (vis_key)
                vis_key = round((vis_key), 5)
                vis_xiuzheng = vis_key * (1 + (0.1875 * re)-(0.0083*re*re))
                ...
                tkinter.messagebox.showerror('Error', '雷诺系数过大')
                re_key = str(re_key)
                vis_key = str(vis_key)
                var_vis.set('粘滞系数为：' + vis_key)
            ...
            tkinter.messagebox.showerror('Error', '数据有问题')
            vis_key = str(vis_key)
    ...
    def calculate_re():
        ...
        if ((t == 'xx.xx') | (L == 'xx.xx') | (rho == 'xx.xx') | (rhoye == 'xx.xx') | (d == 'xx.xx') | (
            ...
            if((re_key<=0.1)&(re_key>=0)):
                var_re.set('雷诺系数' + str(re_key)+'不需要修正')

            elif((re_key >0.1) & (re_key <= 1)):
                var_re.set('雷诺系数' + str(re_key) + '需要一级修正')
            elif((re_key >= 1.0) &(re_key <= 2.0)):
                var_re.set('雷诺系数' + str(re_key) + '需要二级修正')
            else:
                tkinter.messagebox.showerror('Error', '雷诺系数过大或过小')
                var_re.set('雷诺系数' + str(re_key) + '雷诺系数过大或过小')
